//! [`egui`] bindings for [`tao`].
//!
//! This is a modified copy of `egui-tao`.
...
/// Some tao enums are non_exhaustive but the winit counterpart is not.
///
/// Panic if a new variant is found.
#[track_caller]
fn expect_exhaustive() -> ! {
    panic!("unexpected variant that should have been covered")
}
...
pub fn screen_size_in_pixels(window: &Window) -> egui::Vec2 {
    let size = if cfg!(target_os = "ios") {
        ...
        // That will be easier once https://github.com/rust-windowing/tao/pull/3890 lands
        window.outer_size()
    } else {
        window.inner_size()
    };
...
impl State {
    ...
    pub fn on_window_event(
        ...
        match event {
            ...
            WindowEvent::KeyboardInput {
                ...
                // See https://github.com/rust-windowing/tao/issues/3543
    ...
    fn on_keyboard_input(&mut self, event: &tao::event::KeyEvent) {
        ...
        let logical_key = key_from_tao_key(winit_logical_key);
    ...
    pub fn handle_platform_output(
        ...
        if let Some(ime) = ime {
                ...
                || self.egui_ctx.input(|i| !i.events.is_empty())
            {
                ...
                window.set_ime_position(tao::dpi::PhysicalPosition {
                    x: ime_rect_px.min.x,
                    y: ime_rect_px.min.y,
                });
            }
...
fn translate_mouse_button(button: tao::event::MouseButton) -> Option<egui::PointerButton> {
    match button {
        ...
        tao::event::MouseButton::Other(0) => Some(egui::PointerButton::Extra1),
        tao::event::MouseButton::Other(1) => Some(egui::PointerButton::Extra2),
...
fn key_from_tao_key(key: &tao::keyboard::Key) -> Option<egui::Key> {
    use egui::Key;
    use tao::keyboard::Key as TKey;

    Some(match key {
        TKey::Character(str) => return egui::Key::from_name(str),
        TKey::Unidentified(_) | TKey::Dead(_) => return None,
        TKey::Enter => Key::Enter,
        TKey::Tab => Key::Tab,
        TKey::ArrowDown => Key::ArrowDown,
        TKey::ArrowLeft => Key::ArrowLeft,
        TKey::ArrowRight => Key::ArrowRight,
        TKey::ArrowUp => Key::ArrowUp,
        TKey::End => Key::End,
        TKey::Home => Key::Home,
        ...
        TKey::BrowserBack => Key::BrowserBack,
        _ => {
            tracing::trace!("Unknown key: {key:?}");
            return None;
        }
    })
}
...
fn process_viewport_command(
    ...
    tracing::trace!("Processing ViewportCommand::{command:?}");
    ...
    match command {
        ...
        ViewportCommand::StartDrag => {
            // If `.is_focused()` is not checked on x11 the input will be permanently taken until the app is killed!
        ...
        ViewportCommand::Transparent(v) => {
            tracing::warn!("unimplemented set window transparent to {v} command")
        }
        ...
        ViewportCommand::ResizeIncrements(s) => {
            tracing::warn!("unimplemented set resize increments to {s:?} command")
        }
        ...
        ViewportCommand::EnableButtons {
            ...
            minimized,
            maximize,
            "unimplemented enable buttons command"
        ),
        ...
        ViewportCommand::WindowLevel(l) => match l {
            egui::viewport::WindowLevel::AlwaysOnBottom => window.set_always_on_bottom(true),
            egui::viewport::WindowLevel::AlwaysOnTop => window.set_always_on_top(true),
        ...
        ViewportCommand::IMEAllowed(v) => {
            tracing::warn!("unimplemented IME allowed to {v} command")
        }
        ViewportCommand::IMEPurpose(p) => {
            tracing::warn!("unimplemented IME purpose to {p:?} command")
        }
        ...
        ViewportCommand::CursorGrab(o) => {
            if let Err(err) = window.set_cursor_grab(match o {
                ...
                egui::viewport::CursorGrab::Confined | egui::viewport::CursorGrab::Locked => true,
            }) {
                tracing::warn!("{command:?}: {err}");
            }
        }
        ...
        ViewportCommand::MousePassthrough(passthrough) => {
            if let Err(err) = window.set_ignore_cursor_events(passthrough) {
                tracing::warn!("{command:?}: {err}");
            }
        }
...
pub fn create_window<T>(
    ...
    let window = create_tao_window(egui_ctx, viewport_builder.clone(), event_loop)?;
    apply_viewport_builder_to_window(egui_ctx, &window, viewport_builder);
    Ok(window)
}

pub fn create_tao_window<T>(
    egui_ctx: &egui::Context,
    viewport_builder: ViewportBuilder,
    ...
    let mut window = tao::window::WindowBuilder::new()
        ...
        .with_focused(active.unwrap_or(true));

    window = match window_level.unwrap_or_default() {
        egui::viewport::WindowLevel::AlwaysOnBottom => window.with_always_on_bottom(true),
        egui::viewport::WindowLevel::AlwaysOnTop => window.with_always_on_top(true),
        egui::viewport::WindowLevel::Normal => window,
    };
    ...
    // https://github.com/rust-windowing/tao/issues/4266
    #[expect(
    ...
    #[cfg(not(target_os = "ios"))]
    {
        ...
        if let Some(pos) = position {
            window = window.with_position(LogicalPosition::new(
                zoom_factor * pos.x,
                zoom_factor * pos.y,
            ));
        }
    ...
    if let Some(icon) = icon {
        let winit_icon = to_tao_icon(&icon);
        window = window.with_window_icon(winit_icon);
    }

    #[cfg(all(feature = "wayland", target_os = "linux"))]
    if let Some(app_id) = _app_id {
        use tao::platform::wayland::WindowAttributesExtWayland as _;
    ...
    #[cfg(all(feature = "x11", target_os = "linux"))]
    {
        if let Some(window_type) = _window_type {
            use tao::platform::x11::{WindowAttributesExtX11 as _, WindowType};
    ...
    {
        use tao::platform::windows::WindowAttributesExtWindows as _;
...
pub fn apply_viewport_builder_to_window(
    ...
    if let Some(mouse_passthrough) = builder.mouse_passthrough {
        if let Err(err) = window.set_ignore_cursor_events(mouse_passthrough) {
            tracing::warn!("set_cursor_hittest failed: {err}");
        }
    }
...
pub fn short_window_event_description(event: &tao::event::WindowEvent) -> &'static str {
    ...
    match event {
        ...
        WindowEvent::ReceivedImeText(..) => "WindowEvent::ReceivedImeText",
        WindowEvent::DecorationsClick => "WindowEvent::DecorationsClick",
        _ => expect_exhaustive(),
    }
}
