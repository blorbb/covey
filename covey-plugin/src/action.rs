...
#[derive(Debug, Clone)]
pub struct Action(covey_proto::plugin_response::Action);

impl Action {
    pub(crate) fn into_proto(self) -> covey_proto::plugin_response::Action {
        self.0
    }
}
...
impl Action {
    pub fn close() -> Self {
        Self(covey_proto::plugin_response::Action::Close)
    }

    pub fn copy(str: impl Into<String>) -> Self {
        Self(covey_proto::plugin_response::Action::Copy(str.into()))
    }

    pub fn set_input(input: impl Into<Input>) -> Self {
        Self(covey_proto::plugin_response::Action::SetInput(
            input.into().into_proto(),
        ))
    }

    pub fn display_error(err: impl std::fmt::Display) -> Self {
        Self(covey_proto::plugin_response::Action::DisplayError(
            err.to_string(),
        ))
    }
}
